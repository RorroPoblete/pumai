      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={k.label} className="card-gradient border border-[rgba(139,92,246,0.1)] rounded-xl p-5">
              <div className="text-xs text-[#71717A] uppercase tracking-wider mb-2">{k.label}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="card-gradient border border-[rgba(139,92,246,0.1)] rounded-xl p-5">
            <h3 className="text-sm font-semibold text-white mb-6">Daily Conversations</h3>
            <div className="flex items-end justify-between gap-3 h-48">
              {chart.conversationsPerDay.map((d) => (
                <div key={d.day} className="flex-1 flex flex-col items-center gap-2">
                  ...
                  />
                  <span className="text-xs text-[#71717A]">{d.day}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Sentiment */}
          <div className="card-gradient border border-[rgba(139,92,246,0.1)] rounded-xl p-5">
            <h3 className="text-sm font-semibold text-white mb-6">Sentiment Analysis</h3>
              ...
              <div className="relative w-36 h-36">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-xl font-black text-white">{chart.sentimentBreakdown.positive}%</div>
                    <div className="text-[10px] text-[#71717A]">Positive</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <tbody>
                {agents.map((a) => (
                  <tr key={a.id} className="border-b border-[rgba(255,255,255,0.03)] hover:bg-[rgba(139,92,246,0.04)] transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-xs text-[#A1A1AA]">{a.industry}</td>
                    <td className="px-5 py-3.5 text-sm font-semibold text-white">{a.conversationsToday}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-[#A1A1AA]">{a.conversionRate}%</span>
                      </div>
                    </td>
                  </tr>
                ))}
