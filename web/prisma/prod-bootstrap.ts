...
// Populates prod DB with PumAI's own business: 4 channel subscriptions,
// 5 demo agents, and the landing webchat widget (wk_pumai_landing).
//
// Usage (with cloud-sql-proxy running on localhost:15432 + ADC):
//   DATABASE_URL="postgresql://pumai:PASS@localhost:15432/pumai" \
//   CHANNEL_CRED_KEY="<prod base64 key from Secret Manager>" \
//   ADMIN_EMAIL="admin@pumai.com.au" \
//   npx tsx prisma/prod-bootstrap.ts
//
// Idempotent: every write is an upsert. Safe to run multiple times.
...
async function main() {
  ...
  console.log(`🌱 Prod bootstrap for ${adminEmail}`);

  const admin = await prisma.user.findUnique({ where: { email: adminEmail } });
  if (!admin) throw new Error(`Admin user ${adminEmail} not found. Create it first.`);
  console.log(`  ✓ Admin found: ${admin.id}`);

  // ─── PumAI business owned by admin ───
  ...
  // ─── Subscriptions (all 4 channels, GROWTH) ───
  ...
  console.log(`  ✓ Subscriptions: GROWTH × 4 channels`);

  // ─── Membership (admin as OWNER) ───
  await prisma.businessMember.upsert({
    where: { userId_businessId: { userId: admin.id, businessId: business.id } },
    update: { role: "OWNER" },
    create: { userId: admin.id, businessId: business.id, role: "OWNER" },
  });
  ...
  if (existingAgents.length === 0) {
    agents = await Promise.all(
      agentData.map((a) =>
        prisma.agent.create({ data: { ...a, businessId: business.id } }),
      ),
    );
    ...
    console.log(`  ✓ Agents updated: ${existingAgents.length}`);
  }

  // ─── Landing webchat widget ───
  const webchatCredentials = encryptSecret(
    JSON.stringify({
      ...
      allowedOrigins: [
        "https://pumai.com.au",
        "https://www.pumai.com.au",
        "https://pumai-app-822489336766.australia-southeast1.run.app",
      ],
    }),
  );
  ...
  console.log(`  ✓ Webchat widget: wk_pumai_landing (agent: ${agents[0].name})`);
